...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ESP32 Cam QR Code Scanner Stream Web Server SM (Station Mode)
...
const char* password = "Chungtalanguoilichsu";
/* ======================================== */
...
void dumpData(const struct quirc_data *data)
{
  ...
  Serial.printf("-Length: %d\n", data->payload_len);
  Serial.printf("-Payload: %s\n", data->payload);
  String tsData = (const char *)data->payload;

  QRCodeResult = (const char *)data->payload;
  String HOST_NAME = "http://192.168.37.104"; // change to your PC's IP address
  String PATH_NAME   = "/iot/db.php";
  String queryString = "?link="+QRCodeResult;
    HTTPClient http;
